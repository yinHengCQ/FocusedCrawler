...
MODEL_DIR = r"C:\Users\admin\Desktop\java_result"
MODEL_NAME = "graph.pb"
...
def freeze_graph(model_folder):
    ...
    output_node_names = "predict_max_idx" #原模型输出操作节点的名字
    ...
    with tf.Session() as sess:
        ...
        # print("predictions : ", sess.run("predict_max_idx:0", feed_dict={"input_holder:0": [10.0]})) # 测试读出来的模型是否正确，注意这里传入的是输出 和输入 节点的 tensor的名字，不是操作节点的名字
        # print("predictions : ", sess.run(predict_max_idx,feed_dict={input_holder: [get_image_data(img)], keep_prob_holder: 1.0}))  # 测试读出来的模型是否正确，注意这里传入的是输出 和输入 节点的 tensor的名字，不是操作节点的名字
...
if __name__ == '__main__':
    ...
    freeze_graph("C:/Users/admin/Desktop/result")
